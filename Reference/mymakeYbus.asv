function [Ybus, Yf, Yt] = mymakeYbus(baseMVA, bus, branch)
...
Ys = stat ./ (branch(:, BR_R) + 1j * branch(:, BR_X));  %% series admittance 串联部分branch
Bc = stat .* branch(:, BR_B);                           %% line charging susceptance 接地电容branch
tap = ones(nl, 1);                              %% default tap ratio = 1  默认的k=1
i = find(branch(:, TAP));                       %% indices of non-zero tap ratios 找到有变压器的地方
tap(i) = branch(i, TAP);                        %% assign non-zero tap ratios 变压器变比
tap = tap .* exp(1j*pi/180 * branch(:, SHIFT)); %% add phase shifters 变压器变比（加上移相器）

Ytt = Ys + 1j*Bc/2;  % 自导纳branch
...
Ysh = (bus(:, GS) + 1j * bus(:, BS)) / baseMVA; %% vector of shunt admittances 接地电容bus
